...
describe('Cells Reducer', () => {
  it('should create a matrix according to the parameters', () => {
    const response = reducer(
      {},
      {
        ...
        payload: { width: 2, height: 2, totalDemons: 2 }
      }
    );
  ...
  it('should expose all the cells when the game is over and the killer', () => {
    const response = reducer(
      { data: initialData },
      { type: ACTIONS.GAME_OVER, payload: { cellId: '9-9' } }
    );
    expect(response.data['0-0']).to.eql({
      isExposed: true,
      isKiller: false,
      x: 0,
      y: 0
    });
    expect(response.data['0-1']).to.eql({
      isExposed: true,
      isKiller: false,
      x: 0,
      y: 1
    });
    expect(response.data['9-9']).to.eql({
      isExposed: true,
      isKiller: true,
      x: 9,
      y: 9,
  ...
  it('should expose the cell', () => {
    const response = reducer(
      { data: initialData },
      { type: ACTIONS.CELL_EXPOSED, payload: { cellId: '0-0' } }
    );
  ...
  it('should expose empty adjacentCells', () => {
    const response = reducer(
      { data: initialData },
      { type: ACTIONS.EMPTY_CELL_EXPOSED, payload: { cellId: '0-0' } }
    );
    expect(response.data['0-0'].isExposed).to.eql(true);
  ...
  it('should toggle the flag on a cell', () => {
    const response = reducer(
      { data: initialData },
      { type: ACTIONS.CELL_FLAG_TOGGLED, payload: { cellId: '9-9' } }
    );
